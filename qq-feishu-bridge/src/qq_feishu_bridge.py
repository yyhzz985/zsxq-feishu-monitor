...
def init_db():
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS qq_messages(message_id TEXT PRIMARY KEY, group_id INTEGER, sender_name TEXT, msg_type TEXT, content TEXT, status TEXT DEFAULT 'received', feishu_msg_id TEXT, retry_count INTEGER DEFAULT 0, last_error TEXT, raw_json TEXT, created_at TEXT, updated_at TEXT)")
    # Migration: add raw_json column if missing
    try:
        conn.execute("ALTER TABLE qq_messages ADD COLUMN raw_json TEXT")
    except: pass
...
def save_raw_json(conn, mid, event):
    """Save the original OneBot event JSON for potential retry."""
    try:
        raw = json.dumps(event, ensure_ascii=False)
        conn.execute("UPDATE qq_messages SET raw_json=? WHERE message_id=?", (raw, str(mid)))
        conn.commit()
    except: pass
...
def fs_upload_media(filepath, media_type="image", max_attempts=3):
    """Upload media to Feishu with retry for transient errors."""
    last_err = ""
        ...
        try:
            with urllib.request.urlopen(req, timeout=60, context=ctx_ssl) as r:
                ...
                if d.get("code") == 0:
                    ...
                    if attempt > 0:
                        log("  upload retry %d succeeded" % attempt)
                    return True, key
                last_err = str(d)
        except Exception as e:
            last_err = str(e)
            err_lower = last_err.lower()
            if any(kw in err_lower for kw in ['temporary failure', 'timeout', 'connection', 'errno -3', 'errno 110', 'errno 111']):
                if attempt < max_attempts - 1:
                    wait = (attempt + 1) * 2
                    log("  upload retry %d/%d in %ds: %s" % (attempt + 1, max_attempts, wait, last_err[:80]))
                    time.sleep(wait)
                    continue
        break
...
def download_qq_url(url, max_attempts=3):
    """Download from QQ CDN with retry for transient errors."""
    last_err = ""
    for attempt in range(max_attempts):
        try:
            ...
            last_err = str(e)
            err_lower = last_err.lower()
            if any(kw in err_lower for kw in ['temporary failure', 'timeout', 'connection', 'errno -3', 'errno 110']):
                if attempt < max_attempts - 1:
                    time.sleep((attempt + 1) * 2)
                    continue
            break
    log("  download err: " + last_err)
    return None
...
async def retry_failed_messages(conn):
    """Periodically retry failed messages that haven't exceeded max_retries."""
    await asyncio.sleep(60)
    while True:
        try:
            max_r = get_max_retries()
            cutoff = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time() - 1800))
            rows = conn.execute(
                "SELECT message_id, group_id, raw_json FROM qq_messages WHERE status='failed' AND retry_count < ? AND updated_at > ? ORDER BY updated_at",
                (max_r, cutoff)
            ).fetchall()
            for row in rows:
                if not row["raw_json"]:
                ...
                try:
                    ...
                    log("  retry msg %s gid=%s" % (row["message_id"], row["group_id"]))
                    conn.execute("UPDATE qq_messages SET status='received', updated_at=? WHERE message_id=?", 
                               (now_text(), row["message_id"]))
                    conn.commit()
                    await process_message(None, event, conn)
                except Exception as e:
                    log("  retry err: " + str(e)[:100])
        except Exception as e:
            log("  retry loop err: " + str(e)[:100])
        await asyncio.sleep(60)
...
async def main_loop():
    ...
    retry_task = asyncio.create_task(retry_failed_messages(conn))
    while True:
        try:
